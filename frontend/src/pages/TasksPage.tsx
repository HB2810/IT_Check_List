              ...
              <div className="space-y-1 flex-1">
                ...
                {item.status === 'FAULT' && (
                  <div className="p-2.5 rounded-xl bg-white border border-red-200 text-red-800 text-xs mt-2 space-y-2">
                    ...
                    <button
                      onClick={() => {
                        let suggestedItem = 'inv-1'; // default HP toner
                        const lowercaseName = item.name.toLowerCase();
                        if (lowercaseName.includes('printer') || lowercaseName.includes('barcode')) {
                          suggestedItem = 'inv-2'; // receipt rolls
                          ...
                          suggestedItem = 'inv-5'; // cable
                        } else if (lowercaseName.includes('ups') || lowercaseName.includes('battery')) {
                          suggestedItem = 'inv-6'; // UPS battery
                        } else if (lowercaseName.includes('ram') || lowercaseName.includes('memory') || lowercaseName.includes('server')) {
                          suggestedItem = 'inv-7'; // RAM Module
                        } else if (lowercaseName.includes('windows') || lowercaseName.includes('license')) {
                          suggestedItem = 'inv-8'; // Windows CAL
                        } else if (lowercaseName.includes('office') || lowercaseName.includes('email') || lowercaseName.includes('o365')) {
                          suggestedItem = 'inv-9'; // Office 365
                        }
                        
                        navigate('/inventory', { 
                          state: { 
                            requestItemId: suggestedItem,
                            purpose: `Spare part / License required for checklist fault on: ${item.name} (${item.faultNote})`
                          } 
                        });
                      }}
